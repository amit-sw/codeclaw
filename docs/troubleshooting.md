...
## Config Errors
**Symptom**: `openclaw doctor` fails or gateway crashes on startup.
- Ensure `~/.openclaw/openclaw.toml` exists and matches `docs/openclaw.example.toml`.
...
- Approve via CLI: `openclaw tools allow <tool>`
...
- Confirm that sessions exist via `openclaw sessions list --agent <id>`.

## Telegram Not Responding
**Symptom**: Bot does not reply.
- Confirm `telegram.bot_token` is correct.
- Ensure the poller is running (`python -m openclaw.telegram`).
- Check connectivity to `api.telegram.org`.

## Tests Failing
**Symptom**: `openclaw test` fails.
